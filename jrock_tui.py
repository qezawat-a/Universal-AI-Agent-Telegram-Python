...
class JRockApp(App):
            ...
            try:
                ...
                for p in user_providers(self.db, self.user)[:2]:
                    try:
                        ...
                        self.log(f"== {p['name']}: " + ", ".join(ms[:10]))
                    except Exception as ex:
                        self.log(f"== {p['name']} failed: {ex}")
            except Exception as ex:
                self.log(f"⚠️ {ex}")
        elif bid == "b_auto":
            try:
                ...
                self.db.commit()
                self.log(f"✅ auto: {pname} / {model}")
            except Exception as ex:
                self.log(f"⚠️ auto failed: {ex}")
        elif bid == "b_new":
            s = self.memory.new_session(self.db, self.user)
            self.log(f"✅ new session #{s.id}")
    ...
    async def on_input_submitted(self, e: Input.Submitted):
        ...
        self.log(f"J-Rock> {reply or '(empty)'}")

    async def on_input_submitted_cmd(self, text: str):
        if text in ("/help", "/menu"):
            self.log("/soul /provider /models /sessions /resume <id> /newchat + free chat")
        elif text.startswith("/soul"):
            self.log("🧬 " + (self.user.system_prompt or self.soul)[:2000])
            ...
            for s in self.memory.list_sessions(self.db, self.user)[:15]:
                ...
                self.log(f"#{s.id}{mark} — {s.message_count} msgs")
        elif text.startswith("/resume"):
            parts = text.split()
            if len(parts) < 2 or not parts[1].isdigit():
                self.log("Usage: /resume <id> (see /sessions)")
            else:
                s = self.memory.resume_session(self.db, self.user, int(parts[1]))
                self.log(f"✅ resumed #{s.id}" if s else "⛔ no such session")
        else:
            self.log(f"Use CLI for full `/` set: python jrock_cli.py ({text})")
